...
class POSHandler(http.server.SimpleHTTPRequestHandler):
    def do_GET(self):
        ...
        if parsed.path == '/api/contas':
            return self.api_contas()
    ...
    def do_POST(self):
        ...
        if path == '/api/conta/permissoes':
            return self.api_conta_permissoes(params)
    ...
    def api_contas(self):
        conta = self._get_conta_por_token()
        if not conta or conta['tipo'] != 'usuario':
            return self.send_json({'status': 'erro', 'mensagem': 'nao autorizado'}, 403)
        conn = db()
        c = conn.cursor()
        c.execute('SELECT id, nome, email, usuario, tipo, funcionario_id, permissoes, ativo, created_at FROM contas ORDER BY nome')
        linhas = c.fetchall()
        conn.close()
        return self.send_json({'status': 'ok', 'contas': [
            {
                'id': r[0], 'nome': r[1], 'email': r[2],
                'usuario': r[3], 'tipo': r[4],
                'funcionario_id': r[5], 'permissoes': r[6] or '',
                'ativo': r[7], 'created_at': r[8]
            } for r in linhas
        ]})

    def api_conta_permissoes(self, params):
        conta = self._get_conta_por_token()
        if not conta or conta['tipo'] != 'usuario':
            return self.send_json({'status': 'erro', 'mensagem': 'nao autorizado'}, 403)
        conta_id = params.get('id', [''])[0]
        permissoes = params.get('permissoes', [''])[0]
        if not conta_id:
            return self.send_json({'status': 'erro', 'mensagem': 'id obrigatorio'})
        conn = db()
        c = conn.cursor()
        c.execute('UPDATE contas SET permissoes=?, updated_at=datetime("now") WHERE id=?',
                  (permissoes, conta_id))
        conn.commit()
        conn.close()
        return self.send_json({'status': 'ok'})
